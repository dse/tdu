	 ...
	 node_s *kid)
{
  if (parent && kid) {
    ...
    if (parent->nkids) parent->kids[parent->nkids - 1]->islastkid = 0;
	  ...
	  groups_s *groups)
{
  if (node && pathname) {
    ...
    while (1) {
      ...
      /* each iteration in this loop begins by copying the next
	 element (sequence of non-slash characters) of pathname to
	 name.  If the pathname is absolute (i.e., begins with a
	 slash), then name includes a leading slash. */
	...
	while (*p && *p != '/')
	  name[i++] = *(p++);
	name[i] = '\0';
	while (*p == '/') ++p;	/* skip slashes; go to next pathname
				   element */
      }

      /* *after* the previous iteration of this loop processes the
	 last pathname element, name is empty and we're at the
	 destination node; update its contents. */
      
      if (!i) {
	if (node->size >= 0 && node->size != size) {
	  fprintf(stderr,"WARNING!! conflicting entry for %s\n",pathname);
	} else {
	  node->size = size;
	  node->isdupath = 1;
	}
	return;
      ...
      /* if we reach this point we have a path element in name.  p
	 points to a relative path consisting of the rest of the
	 elements.  find or create a child with name as its name and
	 go to that child using the rest of the pathname (p). */

      /* If name is the last path element we assume a file.  If
         filename matches one of the groups, find or create a child
         with the group's name and put the file under it.*/
	...
	if (group) {
      ...
      /* find or create a child that matches this pathname element */
      child = find_or_create_child(node,name);

      /* descend to the next pathname element to reach the destination */
      node = child;
    }
  }
}
	      ...
	      int level)	/* #levels deep, or -1 to expand fully */
{
  ...
  if (node && node->nkids && node->kids && level) {
    ...
    /* if any levels left, recursively call self on each of the kids */    
...
collapse_tree (node_s *node)	/* ptr to tree to collapse */
{
  if (node) {
    ...
    /* First we decrement each ancestor's expand values by the number of
       descendents we will make invisible.  For performance reasons this
...
my_node_number (node_s *node, node_s *root)
{
  if (node) {
	...
	return n;
      ...
      ++n;
      for (i = 0; (i < parent->nkids) && (parent->kids[i] != node); ++i)
	n += (1 + parent->kids[i]->expanded);
      
      if (i == parent->nkids) return -1; /* SHOULD NOT HAPPEN */
	   ...
	   bool isrecursive)	/* go recursive? */
{
  if (node && node->kids && node->nkids) {
    int i;
    if (!fp) fp = node_cmp_unsort; /* provide a default incase fp == NULL */
    node_sort = fp;
    ...
    for (i = 0; i < (node->nkids-1); ++i) node->kids[i]->islastkid = 0;
    node->kids[i]->islastkid = 1;

    /* if operating recursively, work on the kids now */
    if (isrecursive) {
      for (i = 0; i < node->nkids; ++i) {
	tree_sort(node->kids[i],fp,reverse,isrecursive);
      }
    }
  }
}
