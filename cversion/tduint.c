              ...
              display_s ds)
{
  if (ds.fpn)
    ...
    if (node) {
      ...
      if (!node->expanded && node->descendents) { printf(" ..."); }
    }
    printf("\n");
  }
}
                ...
                int level)      /* level of indentation */
{
  ...
  if (node && nodeline >= 0 && nodeline < (1 + node->expanded) && lines) {
    ...
    if (node->expanded && node->kids && node->nkids && lines) {
      ...
      /* Now traverse the children to discover which of those children
         nodeline refers to a node inside. */
      ...
      /* Now that nodeline is less than the number of visible nodes,
         we start displaying nodes from inside node->kids[i].  We may
         have to display nodes from one or more of the next kids as
         well. */
      ...
      while (i < node->nkids && lines > 0) {
        ...
        nodeline = 0;           /* continue at top of next child's
                                   visible tree */
                            ...
                            bool iscursor)
{
  ...
  if (node) {
    ...
    if(node->nkids && !node->expanded) { wprintw_custom(tduwin," ..."); }
  }
}
